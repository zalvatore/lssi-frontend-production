    ...
    class HonorariosList extends Component {
        ...
        renderHonorariosJoined = () => {
            const { honorarios, amount } = this.state;
            console.log('honorarios', honorarios);
            console.log('groupAndSumHonorarios', amount);
        
            // Function to group and sum honorarios by honorario
            const HonorariosCombinedAmount = () => {
                const HonorariosAmount = {};
                ...
                for (const key in honorarios) {
                    if (honorarios.hasOwnProperty(key)) {
                        ...
                        if (!isNaN(amountValue)) {
                            HonorariosAmount[key] = {
                                instructor: honorarios[key], // include honorario name
                                totalIncome: (HonorariosAmount[key]?.sum || 0) + amountValue // accumulate sum
                            };}}}
                return HonorariosAmount;
            };
        
            // Grouped and summed honorarios
            const HonorariosAmount = HonorariosCombinedAmount();
            console.log('HonorariosAmount', HonorariosAmount);
        
            // Render grouped and summed honorarios in a table
            return (
                <div>
                    <h3>Honorarios Grouped</h3>
                    <Table responsive hover striped bordered>
                        ...
                        <tbody>
                            {Object.entries(HonorariosAmount).map(([honorario, { instructor, totalIncome }], index) => (
                                <tr key={index}>
                                    <td>{honorario}</td>
                                    <td>{instructor}</td> {/* Include honorario name */}
                                    <td>{totalIncome}</td>
                                </tr>
                            ))}
                        </tbody>
        ...
        renderInstructorsGrouped = () => {
            const { instructoresProportions } = this.props;
        
            // Function to group and sum instructoresProportions by instructor
            const groupAndSumInstructors = () => {
                const groupedData = {};
        
                // Loop through instructoresProportions
                instructoresProportions.forEach(partner => {
                    const instructor = partner.instructor;
        
                    // If instructor doesn't exist in groupedData, initialize it
                    if (!groupedData[instructor]) {
                        groupedData[instructor] = {
                            instructor: instructor,
                            totalIncome: 0
                        };}
                    // Add partner.totalIncome to the corresponding instructor
                    groupedData[instructor].totalIncome += partner.totalIncome;
                });
            ...
            // Grouped and summed instructoresProportions
            const groupedInstructors = groupAndSumInstructors();
        
            // Render grouped and summed instructoresProportions
            return (
                <div>
                    <h3>Grouped Instructors</h3>
                    <Table responsive hover striped bordered>
                        <thead>
                            <tr>
                                <th>Honorario</th>
                        ...
                        <tbody>
                            {groupedInstructors.map((instructor, index) => (
                                <tr key={index}>
                                    <td>{instructor.instructor}</td>
                                    <td>{instructor.totalIncome.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</td>
                                </tr>
                            ))}
                        </tbody>
                    </Table>
        ...
        render() {
            ...
            const combinedData = [...this.props.instructoresProportions, ...Object.entries(this.state.groupedByInstructor).map(([instructor, totalIncome]) => ({ instructor, totalIncome }))];
            ...
            return(
                ...
                <div>
                    ...
                    <Table responsive hover striped bordered>
                        ...
                        <tbody>
                            ...
                            <tr key='6' className="table-warning">
                                ...
                                <td>{amount.kickoff?.toLocaleString('en-US', 
                                    { style: 'currency', currency: 'USD' })}</td>
                            </tr>
                            ...
                            <tr key='8' className="table-warning">
                                ...
                                <td>{amount.reunioncierre?.toLocaleString('en-US', 
                                    { style: 'currency', currency: 'USD' })}</td>
                            </tr>
                            <tr key='10'>
                                ...
                                <td>{amount.coaching?.toLocaleString('en-US', 
                                    { style: 'currency', currency: 'USD' })}</td>
                            </tr>
                    ...
                    <hr />
                    <div>
                        {this.renderHonorariosJoined()}
                        {this.renderInstructorsGrouped()}
                    </div>
                    <div>
                        {this.renderGroupedData(combinedData, 'instructor', 'totalIncome', value => value.toLocaleString('en-US', { style: 'currency', currency: 'USD' }))}
                    </div>
                    <hr />
                    <div>
                        <h2>Utilidad LSSI {utilidad_lssi?.toLocaleString('en-US', 
